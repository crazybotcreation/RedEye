...
const MAX_USAGE_PER_KEY = 9500; // Leave room from 10,000 to avoid cutoff

function getNextValidKey() {
  for (let i = 0; i < apiKeys.length; i++) {
    const key = apiKeys[currentKeyIndex];
    if (usagePerKey.get(key) < MAX_USAGE_PER_KEY) {
      return key;
    }
    currentKeyIndex = (currentKeyIndex + 1) % apiKeys.length;
  }
  return null; // All keys exceeded
}
...
export default async function postLatestVideo(client, userId, channelId, youtubeChannelId) {
  try {
    ...
    const apiUrl = `https://www.googleapis.com/youtube/v3/search?key=${apiKey}&channelId=${youtubeChannelId}&part=snippet&order=date&maxResults=1&type=video`;
    ...
    if (!video || !video.id?.videoId) {
      return console.warn(`⚠️ No videos found for channel ${youtubeChannelId}`);
    }
